...
###################################################################
# CORRECTS MVP DATA BASED OFF OF SPEEDOMETER AND REARVIEW         #
###################################################################
...
for i in range(len(dir_list)-1):
    if inds[i] == 1:
        data = pandas.read_csv(mvp+ '\\' + str(dir_list[k+1])).to_numpy()
        validData = cb.dataClean(data)

        corrData, isOutlier[k], peaks = cb.SPRVCorr(validData)
