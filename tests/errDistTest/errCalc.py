...
for i in range(len(dir_list)-1):
    if inds[i]==1: # only performs correction for data marked as Good
        ...
        difs = cb.relVectors(coms[0, :], coms[1:, :])
        err = cb.meanComCorr(avgComs, difs, vars)-coms[0, :] # since original data is already considered Good, any correction is the error
        ...
        plt.text(50, 100, 'Correction Vector: [' + str(round(err[0], 2)) + ', ' + str(round(err[1], 2)) + ']', fontsize='small')
        plt.text(50, 50, 'Correction Dist: ' + str(round(errDist[k], 2)), fontsize='small')
